...
public class Session {
	...
	protected final void signTxPacket(SMBPacket pkt) {
		...
		if ( Debug.EnableInfo && hasDebugOption(DBGSigning)) {
			Debug.println("Sign request " + PacketType.getCommandName(pkt.getCommand()) + ", seq=" + seqNo + ", signature = 0x"
	...
	protected final void verifyRxPacket(SMBPacket pkt)
		...
		if ( Debug.EnableInfo && hasDebugOption(DBGSigning)) {
			Debug.println("Verify sign " + PacketType.getCommandName(pkt.getCommand()) + ", seq=" + seqNo + ", signature = 0x"
