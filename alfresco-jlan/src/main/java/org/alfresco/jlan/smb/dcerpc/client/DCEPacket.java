...
public class DCEPacket extends TransPacket {
	...
	public final void initializeDCEWrite(int handle, int flags, int callId, int fragLen) {
		...
		setCommand(PacketType.WriteAndX);
		setFlags(0);
		setFlags2(SMBPacket.FLG2_LONGERRORCODE);
		...
		setAndXCommand(PacketType.NoChainedCommand);
		setParameter(1, 0);
	 ...
	 * @param callId int
	 */
	public final void initializeDCEBind(int pipeHandle, int maxTx, int maxRx, int pipeId, int callId) {
